    ...
    def __init__(self, in_channels):
        ...
        for i in range(num_branchs):
            if i != num_branchs - 1:
                projects.append(
                    nn.Sequential(
                        nn.Conv2d(self.in_channels[i], self.in_channels[i], kernel_size=3, stride=1, padding=1,
...
class ShuffleUnit(nn.Module):
    ...
    def __init__(self,
        ...
        self.branch2 = nn.Sequential(
            nn.Conv2d(in_channels if (self.stride > 1) else branch_features, branch_features, kernel_size=1, stride=1,
                      padding=0),
                    ...
                    for k in range(i - j):
                        if k == i - j - 1:
                            conv_downsamples.append(
                                nn.Sequential(
                                    nn.Conv2d(in_channels[j], in_channels[j], kernel_size=3, stride=2, padding=1,
                                              groups=in_channels[j], bias=False),
                                    nn.BatchNorm2d(in_channels[j]),
                                    nn.Conv2d(in_channels[j], in_channels[i], kernel_size=1, stride=1, padding=0,
                                              bias=False),
                                    nn.BatchNorm2d(in_channels[i]),
                                )
                            )
                            ...
                            conv_downsamples.append(
                                nn.Sequential(
                                    nn.Conv2d(in_channels[j], in_channels[j], kernel_size=3, stride=2, padding=1,
                                              groups=in_channels[j], bias=False),
                                    nn.BatchNorm2d(in_channels[j]),
                                    nn.Conv2d(in_channels[j], in_channels[j], kernel_size=1, stride=1, padding=0,
                                              bias=False),
                                    nn.BatchNorm2d(in_channels[j]),
...
class LiteHRNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.with_cp = with_cp
        self.bn_momentum = self.extra['bn_momentum']
        self.num_classes = self.cfg['DATASET']['NUM_CLASSES']
        self.in_channels = self.cfg['DATASET']['NUM_INPUT_CHANNELS']
        ...
        self.with_deconv_layers = self.extra['with_deconv_layers']

        if self.with_deconv_layers:
            self.inplanes = num_channels_last[0]
            self.deconv_with_bias = self.extra['deconv_spec']['deconv_with_bias']
            self.deconv_layers = self._make_deconv_layer(
                self.extra['deconv_spec']['num_deconv_layers'],
                self.extra['deconv_spec']['num_deconv_filters'],
                self.extra['deconv_spec']['num_deconv_kernels'],
            )

        self.with_final_layer = self.extra['with_final_layer']

        if self.with_final_layer:
            self.final_layer = nn.Conv2d(
                in_channels=self.extra['deconv_spec']['num_deconv_filters'][-1],
                out_channels=self.num_classes,
                kernel_size=self.extra['final_layer_spec']['final_conv_kernel'],
                stride=1,
                padding=1 if self.extra['final_layer_spec']['final_conv_kernel'] == 3 else 0
            )
            ...
            if i < num_branches_pre:
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    transition_layers.append(
                        nn.Sequential(
                            nn.Conv2d(num_channels_pre_layer[i], num_channels_pre_layer[i], kernel_size=3, stride=1,
                                      padding=1, groups=num_channels_pre_layer[i], bias=False),
                            nn.BatchNorm2d(num_channels_pre_layer[i]),
                            nn.Conv2d(num_channels_pre_layer[i], num_channels_cur_layer[i], kernel_size=1, stride=1,
...
def get_seg_model(cfg, **kwargs):
    model = LiteHRNet(cfg=cfg, **kwargs)
    ...
    cfg = dict(
        ...
        MODEL=dict(
            EXTRA=dict(
                ...
                with_final_layer=True,
                final_layer_spec=dict(
                    final_conv_kernel=3,
                ),
            ),
        )
    )
    model = LiteHRNet(cfg=cfg)
    model.eval()
    # print(model)
    image = torch.Tensor(10, 9, 320, 320)
    a = time.time()
    outs = model(image)
    b = time.time() - a
    print(b)
