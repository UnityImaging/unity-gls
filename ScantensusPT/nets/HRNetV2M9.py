...
@torch.jit.script
def mish_function(x: torch.Tensor):
    y = torch.exp(-x)
    z = 1 + 2 * y
    return x * z / (z + 2 * y * y)


def swish_function(x: torch.Tensor):
        ...
        for i in range(num_branches if self.multi_scale_output else 1):
            ...
            for j in range(num_branches):
                if j > i:
                    ...
                    conv3x3s = []
                    for k in range(j, i):
                        num_inchannels_conv3x3 = num_inchannels[k]
                        num_outchannels_conv3x3 = num_inchannels[k+1]
                        if k < j:
                            conv3x3s.append(nn.Sequential(
                                nn.Conv2d(num_inchannels_conv3x3,
